...
const toCommaSeparated = fp.reduce((acc: string, sub: { SubscriptionName: string }) => `${sub.SubscriptionName}${acc ? `, ${acc}` : ""}`, "");
const drawOptions = {
...
const listSubsAllTopics = async () => {
    ...
    const listTopicsAsync = promisify(serviceBusService.listTopics).bind(serviceBusService);
    const listSubscriptionsAsync = promisify(serviceBusService.listSubscriptions).bind(serviceBusService);
